...
@section('page-title', 'Adaugă Produs Nou')

@section('content')
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div class="mb-6">
            <h1 class="text-2xl font-bold text-gray-900 mb-2">Adaugă Produs Nou</h1>
            <p class="text-gray-600">Completează informațiile pentru noul produs</p>
        </div>
        ...
        <form method="POST" action="{{ route('products.store') }}" class="space-y-6">
            ...
            <div>
                ...
                @error('price')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
                <p class="mt-1 text-sm text-gray-500">Prețul va fi salvat pentru fiecare sesiune la momentul adăugării</p>
            </div>

            <div>
                <label for="tax_group" class="block text-sm font-medium text-gray-700 mb-2">
                    Grupă TVA Fiscală <span class="text-red-500">*</span>
                </label>
                <select id="tax_group"
                        name="tax_group"
                        class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 @error('tax_group') border-red-500 @enderror"
                        required>
                    @for($group = 1; $group <= 8; $group++)
                        <option value="{{ $group }}" {{ (int) old('tax_group', 2) === $group ? 'selected' : '' }}>
                            Grupa {{ $group }}
                        </option>
                    @endfor
                </select>
                @error('tax_group')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
                <p class="mt-1 text-sm text-gray-500">Configurează grupa fiscală folosită la emiterea bonului.</p>
            </div>
